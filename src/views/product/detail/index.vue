...
const saveFormValue = (newValue: any, currentInfo: any) => {
    ...
    Object.entries(currentInfo.formValue).forEach(([key, value]) => {
        if (key.endsWith('Attachments') && Array.isArray(value)) {
            value = value.map(i => ({id:i.response?.id,filename:i.response?.originalName,url:i.response?.url}));
        }else if(key.endsWith('Image')&&Array.isArray(value)){
            const currentImg = value[0]
            value = {id:currentImg?.response.id,filename:currentImg.response?.originalName,url:currentImg.response?.url}
        }
        otherData[key]  = value
    })
        ...
        for (const key in res) {
            if (Object.prototype.hasOwnProperty.call(item.formValue, key)) {
                if(key.endsWith('Image')&&res[key]){
                    res[key] = [{...res[key],name:res[key]?.filename,response:{...res[key],originalName:res[key]?.filename}}]
                }else if(key.endsWith('Attachments')&&Array.isArray(res[key])){
                    res[key] = res[key].map(r=>({...r,name:r.filename,response:{...r,originalName:r.filename}}))
                }
                item.formValue[key] = res[key] ? res[key] : (key.endsWith('Image') || key.endsWith('Attachments') ? [] : '');
            }
        }
        return item
    })
            ...
            <a-tabs v-model:activeKey="activeKey" size="large" :tabBarGutter="200">
                <a-tab-pane v-for="item in tabs" :key="item.key" :tab="item.tab">
                    <FormGenerator :modelValue="item.formValue" :fields="item.fields"
                        :componentAttrs="item.componentAttrs" :className="item.className"
                        :buttonContent="item.buttonContent"
                        @on-finish="newFormValue => saveFormValue(newFormValue, item)">
                        <template #specificationAttachments-file-upload="props">
                            <BaseFileListUpload :fileList="item.formValue.specificationAttachments"
                                @on-change="file => handleFileChange(file, item, props)"></BaseFileListUpload>
                        </template>
                        <template #installationImage-img-upload="props">
                            <BaseImageUpload :imageList="item.formValue.installationImage" :maxCount="1"
                                @on-change="file => handleImageChange(file, item, props)"></BaseImageUpload>
                        </template>
                        <template #installationAttachments-file-upload="props">
                            <BaseFileListUpload :fileList="item.formValue.installationAttachments"
                                @on-change="file => handleFileChange(file, item, props)"></BaseFileListUpload>
                        </template>
                        ...
                        <template #performanceAttachments-file-upload="props">
                            <BaseFileListUpload :fileList="item.formValue.performanceAttachments"
                                @on-change="file => handleFileChange(file, item, props)"></BaseFileListUpload>
                        </template>
                        ...
                        <template #engineeringAttachments-file-upload="props">
                            <BaseFileListUpload :fileList="item.formValue.engineeringAttachments"
                                @on-change="file => handleFileChange(file, item, props)"></BaseFileListUpload>
                        </template>
                    </FormGenerator>
                </a-tab-pane>
